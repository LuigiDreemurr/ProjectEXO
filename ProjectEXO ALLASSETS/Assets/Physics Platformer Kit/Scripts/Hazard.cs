...
public class Hazard : MonoBehaviour 
{
    ...
    private Throwing pushable;

    private SuitUpgrades SuitControl;

    public enum hazardType { Fire = 1, Radiation = 2, Saw = 4, Laser };
	...
	void Awake()
	{
        ...
        pushable = GameObject.FindGameObjectsWithTag("Player")[0].GetComponent<Throwing>();
	...
	void OnCollisionEnter(Collision col)
	{
        ...
        foreach (string tag in effectedTags)
            if ((col.transform.tag == tag && (int)currentHazard != (int)SuitControl.currentSuit) || (col.transform.tag == "Pushable" && currentHazard == hazardType.Laser))
			{
                Debug.Log(col.transform.tag);
                Debug.Log(tag);

                if(pushable.heldObj != null)
                {
                    pushable.DropPushable();
                }
    ...
    void OnCollisionStay(Collision col)
    {
        ...
        foreach (string tag in effectedTags)
            if (col.transform.tag == tag && (int)currentHazard != (int)SuitControl.currentSuit)
            {
                Debug.Log(col.transform.tag);
                Debug.Log(tag);
	...
	void OnTriggerEnter(Collider other)
	{
		if(!triggerEnter)
			return;
		foreach(string tag in effectedTags)
			if(other.transform.tag == tag && (int)currentHazard != (int)SuitControl.currentSuit)
